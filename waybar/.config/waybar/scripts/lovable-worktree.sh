...
worktree). Hides itself entirely when no lovable-* workspace is
the focused stack.
...
# Reload on the same niri events that ws-tracker reacts to.
...
def render() -> str:
    ...
    if not active or not active.startswith("lovable-") or active == "lovable-deps":
        # Hide module entirely — empty text + "empty" class so CSS can
        # collapse padding/margin to zero.
